...
\caption{Primary 2021 manual-reference validation. (a,b) Single-frame and Bayesian estimates against manual height for all \FieldN{} records; dashed lines mark equality. (c) Within-plant difference in mean absolute error, ordered by single-frame error; negative values favor the Bayesian estimate. (d--f) Trajectories selected by the fixed 10th-, 50th-, and 90th-percentile rule on within-plant benefit, illustrating harm, median behavior, and benefit without residual-based exclusion. Shading is the 95\% latent-height posterior interval. The paired uncertainty is the stationary-camera-row bootstrap interval reported in panel c and Table~\ref{tab:field}.}
\label{fig:field}
\end{figure}
...
Several limitations determine the next experiments. First, the primary manual-reference validation spans 12 stationary-camera rows but only 33 plants in one 2021 season. The secondary 2025 interval test contains 372 forecasts from 11 plant tracks at two cameras, while the separate locked calibration test contains three cameras. Second, annual replanting makes the 2025 cohort temporally later and subject-disjoint from 2024, which supports year-forward validation of calibration and next-image prediction. The 2024--2025 stream has no paired manual plant heights, however, while the 2021 validation uses an archived segmentation extent and a growth field learned from later seasons. The remaining gap is therefore end-to-end manual-reference physical-height validation in a later subject-disjoint year. Third, human validation serves component-specific purposes: 150 plant masks assess image extent, 132 field measurements assess physical height, and Haoming Wang's \ManualPoleAnnotations{} manual traces across \ManualPoleImages{} images audit the 2021 physical-reference geometry. These sources do not estimate inter-reader variation for agronomic landmarks. Fourth, the depth ratio assumes known distances, level ground, negligible roll, and stable camera geometry. Fifth, the released checkpoint permits inference on the curated images, but its complete original training provenance remains only partially documented.
...
The study contains linked data sets with different roles (Table~\ref{tab:design}). The primary physical-height validation uses 2021 manual measurements on five dates and an archived single-frame segmentation extent for every available matched plant-date record across 12 stationary-camera rows. The manual measurements are reference outcomes only. The 2024 development and 2025 locked-test pole images separately evaluate calibration, and a quality-controlled subset supplies a daily image-derived plant stream for the secondary prequential uncertainty check. Plants were newly grown in each season, so no biological plant subject recurs across years. The 2024-to-2025 split is subject-disjoint as well as year-forward, although repeated images within a plant and camera remain dependent. A separate set of 2021 human-annotated images supports out-of-fold detector validation. Haoming Wang's 2021 support-pole annotations provide a completed human audit of the physical installation; they are not treated as the red-band calibration poles.
...
\midrule
2021 field heights & \FieldN{} records; \FieldPlants{} plants; \FieldRows{} stationary-camera rows; \FieldDates{} dates & Manual height and geometrically scaled image extent & Primary manual-reference physical-height validation \\
...
For the primary 2021 manual-reference validation, the per-image observation standard deviation ranges from 12 to 30 cm with recorded daily wind, and the process-height scale is estimated from image increments without manual height. A nonparametric growth-rate field $\hat r(h)$ is learned from 2024--2025 pole-stream increments and transferred to the subject-disjoint 2021 plants. No 2021 manual height enters the single-frame measurement, growth field, filter, noise parameters, or comparator settings. The 2021 data therefore provide cross-season, label-held-out physical-height validation, but the transfer is retrospective in calendar direction; each update remains prefix-causal within its image sequence. The explicit early-to-late evaluation is the secondary 2024 development to subject-disjoint 2025 predictive-interval test.
