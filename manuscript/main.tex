...
\begin{abstract}
Affordable longitudinal field phenotyping from fixed cameras is limited by ambiguous detections and uncertain pixel-to-centimeter calibration. Most pipelines analyze each image separately and apply a longitudinal model only after measurement extraction. We instead feed the Bayesian predictive state into current-image analysis: predicted position guides plant association, and the predicted height distribution can score competing top candidates before the measurement enters the longitudinal update. A robust particle filter updates calibrated plant height and growth. Each fixed-setting update uses images through time $t$ and returns an as-of height distribution. Held-out pole bands evaluated calibration across cameras and years. Independent physical-height validation used 132 manual measurements from 33 plants across 12 stationary-camera rows in 2021, with manual height excluded from fitting and tuning. Among six causal estimators, the particle filter had the lowest mean absolute error (10.97 cm). Its gains over EWMA, running median, and Holt were 4.01, 7.80, and 3.08 cm with row-cluster intervals above zero; its 1.06- and 0.66-cm gains over single-frame and Gaussian filters were uncertain. Posterior 95\% intervals covered 94.7\% of 132 held-out manual heights (75.4-cm mean width); year-forward 2025 predictive coverage was 94.9\% using 2024-derived parameters. In controlled ambiguity, prior-guided candidate scoring reduced latent-height root mean squared error from 10.5 to 6.0 cm, compared with 8.6 cm when filtering began after candidate selection; a matched real-image audit found no candidate conflict. The study supports affordable longitudinal phenotyping while locating the methodological innovation at the image-analysis decision.
\end{abstract}
...
\midrule
2021 field heights & \FieldN{} records; \FieldPlants{} plants; \FieldRows{} stationary-camera rows; \FieldDates{} dates & Manual height and geometrically scaled image extent & Primary independent manual-reference physical-height validation \\
...
Quality-controlled sequential stream & \PoleStreamRows{} records; \PoleStreamPlants{} tracks; \PoleStreamCameras{} cameras; \PoleStreamImages{} images & Red-band calibration and camera geometry & Secondary next-image prediction and interval transfer \\
...
Several limitations determine the next experiments. First, the primary independent validation spans 12 stationary-camera rows but only 33 plants in one 2021 season. The secondary 2025 interval test has 11 plant tracks at two cameras; the locked calibration test has three cameras. Second, the 2024--2025 stream lacks manual plant-height outcomes. The independent 2021 study supplies physical-height truth on five dates through season end, using archived segmentation extents and an external growth field estimated without 2021 labels. Another subject-disjoint manual-reference season would strengthen transport evidence, but is not required for present independence. Third, human validation serves component-specific purposes: 150 plant masks assess image extent, 132 field measurements assess physical height, and Haoming Wang's \ManualPoleAnnotations{} newly completed traces of \ManualPoleImages{} 2021 images audit physical-reference geometry. These sources do not estimate inter-reader variation for agronomic landmarks. Fourth, the depth ratio assumes known distances, level ground, negligible roll, and stable camera geometry. Fifth, the released checkpoint permits inference on the curated images, but its complete original training provenance remains only partially documented. Sixth, source-frame matching recovered positions for 87 archived plant crops; 63 of these map to the primary validation across six camera rows. For the remaining 69 primary records, the archive preserves the derived segmentation extent but not a reconstructable raw-image-to-extent chain.

A publication-scale extension could freeze all settings; evaluate without retuning in additional newly planted, subject-disjoint cohorts with manual reference heights; compare prior-in-image selection with post-hoc filtering using the same detector; and report cluster intervals matching the sampling design. The present 2021 study already supplies independent physical-height validation, and the 2024-to-2025 split validates temporal transfer for calibration and next-image prediction. An extension would add sample size, camera diversity, and external transport evidence.
...
Analysis code, the 61 curated 2021 images, 12 human-annotated support-pole XML files, the imported 199-trace audit table, the released pose checkpoint, candidate predictions, derived data, figure sources, and manuscript source are available in the \href{https://github.com/ChongWangStat/Bayesian-Longitudinal-Maize-Height}{public GitHub repository}. The repository also provides the 87-record partial source-frame coordinate recovery described above. It includes an MIT license for code and a SHA-256 manifest; no separate reuse license has been assigned to the non-code materials. The larger 2024--2025 raw-image archive remains in institutional storage; checksum-identified derived measurements needed for the reported analyses are public.
