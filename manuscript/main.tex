...
\title{Prior-Guided Bayesian Image Analysis for Physically Calibrated Longitudinal Maize Height}
...
\begin{abstract}
Affordable longitudinal field phenotyping from fixed cameras is limited by ambiguous detections and uncertain pixel-to-centimeter calibration. We coupled a running plant-position state for current-image association with physically calibrated measurements and a robust particle filter for plant-specific height and growth. With settings fixed, every update uses only images available through time $t$ and immediately returns an as-of height distribution. Held-out red pole bands evaluated physical calibration across cameras and years. Against 132 manual measurements from all 33 plants in 12 rows, longitudinal estimation reduced mean absolute error from 12.03 to 10.97 cm (paired reduction 1.06 cm; 95\% row-cluster bootstrap interval, -0.16 to 2.39). Its point estimate was also lower than four fixed causal smoothers, including a Gaussian local-linear Kalman filter at 11.62 cm, although their paired difference was uncertain (0.66 cm; -0.43 to 1.79). After all noise scales were converted to target-row units, parameters learned on 2024 data gave 94.9\% coverage with a 97.8-cm mean 95\% width in 372 next-image forecasts from two held-out 2025 cameras. In a controlled ambiguity simulation, using the predicted height distribution during candidate selection reduced latent-height root mean squared error from 10.5 to 6.0 cm, compared with 8.6 cm for filtering after candidate selection. A matched real-image audit found no candidate conflict under the prespecified gates. These results support an affordable, transparent longitudinal workflow while identifying candidate diversity, interval sharpness, and independent manual validation as priorities.
\end{abstract}

\noindent\textbf{Keywords:} longitudinal field phenotyping; maize height; particle filter; position-guided association; stationary camera; uncertainty calibration

\section{Introduction}

Longitudinal plant-height trajectories describe growth rate, developmental timing, and response to environment more directly than a single mature measurement. Repeated manual measurement is costly and can disturb field plots, while fixed red-green-blue cameras can collect the same view throughout a season at comparatively low cost. Existing time-series phenotyping systems have produced useful growth curves in maize and other crops \cite{ge2016temporal,liang2018timeseries,wang2020functional}. The practical obstacle is that a camera produces pixels rather than a physical measurement. Canopy overlap, occlusion, wind, changes in illumination, and the transition from leaf tips to reproductive structures make the top and base of an individual plant ambiguous. A separate reference object can establish scale, but its geometry and usable range must be known. A longitudinal method must also state exactly when information becomes available and how uncertainty changes as new images accumulate.
...
State-space models provide a formal way to carry information through time. Kalman and particle filters are standard tools for sequential estimation \cite{west1997bayesian,durbin2012time,gordon1993particle}. Particle filtering has been used for real-time rice-height estimation from Sentinel-1 images, and a recent Bayesian pattern-matching framework estimates daily rice height from satellite vegetation indices and sparse field measurements \cite{yang2022ricepf,shimda2026bayesianheight}. A Kalman tracker has also been combined with a maize seedling detector for counting \cite{li2022kalman}, while Bayesian adaptive sampling has been applied to affordable time-lapse germination phenotyping \cite{mercier2025bayesian}. We therefore make no broad claim for the first use of Bayesian methods, temporal tracking, or a prior in plant phenotyping. The distinguishing contribution examined here is the specific coupling of a running plant-position state for current-image association, a plant-specific Bayesian height-growth distribution, physical calibration, and as-of uncertainty reporting. In the field implementation, the running position state guides association and base-position refinement. The stronger use of a predicted height distribution to choose among competing top candidates is evaluated as a controlled component in simulation. This distinction keeps the claim aligned with the executed evidence.

The study integrates three elements for longitudinal field phenotyping. First, it uses in-scene physical references and recorded camera geometry, with separate definitions for 2021 camera-support poles and 2024--2025 red-band calibration poles. Second, it performs sequential, outlier-robust Bayesian updating and computes next-image predictive intervals from the full particlewise mixture rather than a Gaussian approximation. Third, it defines online operation as an as-of update: when a new image is appended, the algorithm analyzes the available image prefix and emits an as-of measurement after processing that image; later observations join the pool only when they arrive. The methodological novelty lies in placing a plant-specific Bayesian height-growth distribution inside this longitudinal image-analysis loop, while restricting each empirical claim to the component with an available reference. We evaluate held-out pole bands for calibration, image annotations for detection, simulated latent trajectories for candidate ambiguity, and all available 2021 plant-date field measurements for physical-height agreement.
...
Exact mixture-based predictive evaluation materially changed the interpretation of uncertainty (Table~\ref{tab:uncertainty}). Guessed noise settings covered \GuessedCoverage{}\% of next image-derived measurements with a mean 95\% width of \GuessedWidth{} cm. After the depth correction was applied to every centimeter-scale noise quantity, the 2024-derived model covered \CalibratedCoverage{}\% with a \CalibratedWidth{}-cm mean width over the full stream. On the held-out 2025 subset, coverage was \UncertaintyTestCoverageNinetyFive{}\% with a \UncertaintyTestWidthNinetyFive{}-cm mean width across \UncertaintyTestN{} forecasts from \UncertaintyTestCameras{} cameras. Its weighted interval score was \UncertaintyTestWIS{} cm. Lower nominal levels remained conservative (Supplementary Figure S3), so the near-nominal 95\% result should not be interpreted as complete distributional calibration. The calibrated Bayesian predictive mean had mean absolute error \ForecastBayesMAE{} cm and root mean squared error \ForecastBayesRMSE{} cm, compared with \ForecastLastMAE{}/\ForecastLastRMSE{} cm for the last observation carried forward and \ForecastLinearMAE{}/\ForecastLinearRMSE{} cm for two-point linear extrapolation. These comparisons target the next noisy image measurement, not manual physical height.
...
This study develops a physically interpretable route from an accumulating longitudinal fixed-camera image stream to an as-of plant-height distribution after each image. The key methodological contribution is the coupling: a running plant-position state informs current association and base refinement; physical references convert image extent to centimeters; and a robust particle filter propagates the plant-specific height-growth distribution and its uncertainty. Prior field-maize systems have reused earlier locations for later segmentation and height extraction \cite{gao2022individual,li2023multisource}, and plant vision has used Kalman tracking \cite{li2022kalman}. The contribution is therefore this particular integration and its explicit uncertainty audit rather than priority for temporal guidance itself.
...
Uncertainty is similarly transparent. Applying the depth conversion to the observation, process, and outlier scales corrected a consequential unit mismatch. With parameters estimated from 2024 only, 95\% coverage was 94.9\% on the held-out 2025 stream with a 97.8-cm mean width, while 50\%, 80\%, and 90\% intervals overcovered. Coverage and sharpness together therefore describe a conservative distribution rather than proving perfect calibration. Repeated same-time images, direct uncertainty labels, or a hierarchical design are needed to separate measurement and biological variation and narrow intervals without losing coverage.

Several limitations determine the next experiments. First, the 2021 physical-height study contains only 33 plants in 12 rows, the 2025 interval test contains 372 forecasts from two sequential cameras, and the calibration test contains three cameras. Second, the 2024--2025 stream has no paired manual plant heights, while the 2021 comparison uses an archived segmentation extent and an externally learned growth field. This split validates components but not a single end-to-end system on an independent year. Third, the existing masks define the highest and lowest visible plant pixels; two independent readers have not yet completed the released protocol for the agronomic top, collar, flag leaf, and root landmarks on the \ManualPoleImages{} images. Fourth, the depth ratio assumes known distances, level ground, negligible roll, and stable camera geometry. Fifth, the released checkpoint permits inference on the curated images, but its complete original training provenance remains only partially documented.

A publication-scale confirmation should freeze the detector, physical calibration, growth field, noise settings, and candidate gates on a development year; acquire an independent camera-year with paired plant and pole annotations; compare prior-in-image selection with the same detector followed by post-hoc filtering; and report camera- or row-cluster intervals for both accuracy and coverage. The present results justify that study and provide an auditable implementation, while the observed field improvement and interval width set realistic benchmarks for it.

\section{Materials and Methods}

\subsection{Experimental and technical design}

Figure~\ref{fig:workflow} shows the information flow. Let $\mathcal I_{\leq t}$ be the images available through acquisition time $t$. The image arriving at $t$ is appended to this pool; the running position state from $t-1$ guides association, while the height-growth posterior is propagated before the new measurement is incorporated; detected top and base rows are converted to centimeters; and the updated height-growth posterior is returned. An image acquired after $t$ is absent from this computation. This is the operational meaning of \emph{online} used throughout the paper. The available data were analyzed retrospectively, so we call the evaluation a retrospective validation of a prefix-causal algorithm rather than a historical prospective deployment. Hyperparameters and growth-field estimates must be frozen from development or external data before future prospective use.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{online_bayesian_workflow.pdf}
\caption{Workflow for the as-of image update. Solid blue stages are executed in the field pipeline: running position state, current detection and association, base-position refinement, physical conversion, and robust Bayesian height-growth update. The dashed amber candidate-scoring branch uses the predicted height distribution to choose among alternative top candidates and is evaluated only in the controlled simulation. At time $t$, only $\mathcal I_{\leq t}$ is available; with settings fixed, a later image starts a new update and does not alter an earlier output.}
\label{fig:workflow}
\end{figure}

The study contains linked data sets with different roles (Table~\ref{tab:design}). The 2024 development and 2025 locked-test pole images evaluate calibration. A quality-controlled subset also supplies a daily image-derived plant stream for prequential uncertainty checks. The 2021 field data provide manual heights on five dates and an archived single-frame segmentation extent for all available plant-date records. A separate set of 2021 annotated images supports out-of-fold detector validation. The newly curated 2021 support-pole annotations document the physical installation and provide a fixed asset for future paired plant annotation; they are not treated as the red-band calibration poles.
...
2021 support poles & \ManualPoleXMLFiles{} XML files; \ManualPoleImages{} retained images; \ManualPoleAnnotations{} traces & Nominal \SupportPoleLengthFt{}-ft support pole; ground contact to camera mount/optical-center height & Installation definition and annotation resource \\
...
The 2021 XML files contain labels \texttt{pole1} through \texttt{pole4}. Each label identifies a different stationary-camera support pole visible in that image; the number does not encode depth. The annotated vertical segment runs from ground contact to the nominal camera mounting or optical-center height, \SupportPoleLengthFt{} ft (\SupportPoleLengthCm{} cm). The field records also contain four horizontal layout intervals of \FieldMarkIntervalIn{} in, totaling \AdjacentRowSpacingIn{} in between adjacent rows. These horizontal marks are not tick marks on the support poles. The confirmed duplicate \texttt{C-039\_2021-07-30JPG.JPG} was removed while \texttt{C-039\_2021-07-30.JPG} was retained. The updated curated set has \ManualPoleImages{} unique XML-referenced images, \ManualPoleUsable{} usable traces, and \ManualPoleUnusable{} traces marked unusable (Figure~\ref{fig:supportpole}).
...
\subsection{Image measurements and prior-guided longitudinal association}

For the 2024--2025 stream, a YOLOv8-pose model produces candidate plant instances with top and root keypoints. The first complete frame initializes six left-to-right tracks. At each later frame, observed root positions are matched to running track positions by minimum total horizontal distance, subject to a maximum normalized distance of 0.16. The track position and root row are then updated with a fixed gain of 0.5. The state carried from preceding images therefore guides current association and base refinement. The sequential configuration associated \SequentialMatches{} detections and recovered \SequentialExtraMatches{} associations relative to frozen first-frame anchors; it changed the root used in the physical extent for \PosteriorRootChanged{} matched records. This recursion has the form of an exponentially weighted moving average, or a steady-state Kalman update with fixed gain. The deployed assembler does not estimate position covariance and is not a fully Bayesian neural network.
...
Author contribution roles will be confirmed by all authors and entered before submission.

\subsection*{Funding}
This work was supported by the Plant Sciences Institute, Iowa State University. Grant numbers and awardees, if applicable, will be added before submission.

\subsection*{Conflicts of Interest}
The authors declare that there is no conflict of interest regarding the publication of this article.

\subsection*{Data Availability}
Analysis code, the 61 curated 2021 images, 12 support-pole XML files, the released pose checkpoint, candidate predictions, derived data, independent-annotation packet, figure sources, and manuscript source are available in the \href{https://github.com/ChongWangStat/Bayesian-Longitudinal-Maize-Height}{public GitHub repository}. The repository includes explicit code and data licenses and a SHA-256 manifest. The larger 2024--2025 raw-image archive remains in institutional storage; checksum-identified derived measurements needed for the reported analyses are public.
