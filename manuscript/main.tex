...
\subsection{Secondary temporal transfer of predictive uncertainty}
...
The primary physical-height evidence is the independent 2021 validation across 12 stationary-camera rows, 33 plants, and five dates. Test plants were absent from the 2024--2025 development data. Manual height was excluded from fitting and tuning, every matched record was retained, and the bootstrap resampled whole camera rows to preserve dependence. Mean absolute error decreased by about 1 cm relative to a single frame and by 0.66 cm relative to a Gaussian local-linear Kalman filter, but both camera-row-cluster intervals included no improvement. The result does not support a claim that longitudinal Bayesian filtering uniformly improves height. It does show that the robust particle filter achieved the best point estimate across a broader set of causal baselines without manual-height tuning.
